...
def _load_tribe_reverse_map(source_path: Path) -> Dict[str, int]:
    candidates = [
        ...
        Path(__file__).resolve().parent / "tribe_extracted.csv",
    ]
...
def _st_to_preset(
    ...
    tag_base = int(st_row.get("tag_group_base_id", 0) or 0)
    tags = []
    for group_id in range(tag_base, tag_base + 10):
        ...
        effects = [effects_by_id.get(effect_id, {"effect_id": effect_id}) for effect_id in effect_ids if effect_id]
...
def parse_vs_lua(source_path: Path) -> Dict[str, Any]:
    ...
    effects_by_id = {int(row.get("effect_id", 0)): row for row in effect_rows if int(row.get("effect_id", 0) or 0)}
...
def ensure_vs_json(source_path: Path, output_path: Path) -> Dict[str, Any]:
    def write_effect_csv(payload: Dict[str, Any]) -> None:
        csv_path = output_path.with_name("vs_effect_translation.csv")
        with csv_path.open("w", encoding="utf-8-sig", newline="") as f:
            writer = csv.DictWriter(f, fieldnames=["effect_id", "side", "kind", "sub_id", "value", "name", "description"])
            writer.writeheader()
    ...
    if output_path.exists() and output_path.stat().st_mtime >= source_path.stat().st_mtime:
        with output_path.open("r", encoding="utf-8") as f:
            payload = json.load(f)
        if payload.get("parser_version") == 4 and payload.get("presets") and payload.get("st_count") is not None and payload.get("vs_effect_count") is not None:
