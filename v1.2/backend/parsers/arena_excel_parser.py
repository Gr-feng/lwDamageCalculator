...
def parse_effect_text(text: str) -> List[List[int]]:
    ...
    for element, sub_id in ELEMENT_CHAR_TO_ID.items():
        if f"{element}属性" in text and "伤害下降" in text:
            match = re.search(r"伤害下降(\d+)%", text)
            effects.append([13, sub_id, target, duration, _to_int(match.group(1) if match else 0, 0)])
