...
a = Analysis(
    ...
    binaries=[],
    datas=[
        (str(V1_DIR / "characters.csv"), "."),
        (str(V1_DIR / "datajson"), "datajson"),
        (str(V1_DIR / "gui" / "resources"), "gui/resources"),
        (str(V1_DIR / "tribe_extracted.csv"), "."),
        (str(V1_DIR / "equipment_data.json"), "."),
        (str(V1_DIR / "recommended.csv"), "."),
        (str(V1_DIR / "local_translations.json"), "."),
        (str(V1_DIR / "buff_translation.csv"), "."),
    ],
    hiddenimports=[],
    hookspath=[],
    hooksconfig={},
    runtime_hooks=[str(V1_DIR / "pyi_rth_qt_env.py")],
    excludes=[
...
# PyInstaller's Qt hooks still pull in a lot of optional runtime files.
# Prune them after analysis to keep only the widgets stack we actually use.
...
exe = EXE(
    ...
    icon=[str(V1_DIR / "gui" / "resources" / "app.ico")],
)
