...
from backend.core.character import CharacterInstanceManager, CharacterInstance
from backend.core.battle_op_state import BattleOpStateManager
from backend.core.buff_effect import parse_buff, parse_effect
...
def _get_crit_expectation(mgr: CharacterInstanceManager, attacker: CharacterInstance, defender: CharacterInstance, attack_entry: Dict[str, Any], ctx: DamageContext) -> Dict[str, Any]:
    ...
    crit_def_correction_54 = _get_buff54_crit_def_correction(defender)
    ...
    if killer:
        crit_correction = (1.0 + crit_bonus * killer_bonus_factor) * crit_def_correction_54
        crit_mode = "killer"
    else:
        crit_correction = 1.0 + crit_rate * ((1.0 + crit_bonus) * crit_def_correction_54 - 1.0)
        crit_mode = "normal"
    ...
    return {
        ...
        "crit_def_correction_54": crit_def_correction_54,
...
def _get_buff54_crit_def_correction(defender: CharacterInstance) -> float:
    value = 0.0
    for buff in list(getattr(defender, "buffs", []) or []):
        try:
            if int(getattr(buff, "buff_id", 0)) == 54 and int(getattr(buff, "sub_id", 0)) == 1205001:
                value += float(getattr(buff, "value", 0.0) or 0.0)
        except Exception:
            continue
    if abs(value) <= 1e-9:
        return 1.0
    denominator = max(0.01, 1.0 + value / 100.0)
...
def _has_barrier_status_immunity(char: CharacterInstance, barrier_type: int) -> bool:
    ...
    约定：1/2/4/5 免疫对应异常的负面效果；6/7/8 是受异常时触发增益，不免疫负面。
    """
    ...
    if stats is not None and hasattr(stats, key):
        try:
            return int(getattr(stats, key) or 0) in (1, 2, 4, 5)
        except Exception:
            pass

    info = getattr(char, "character_info", {}) or {}
    try:
        return int(info.get(key, 0) or 0) in (1, 2, 4, 5)
    except Exception:
        return False
...
def _calc_hit_and_crit(mgr: CharacterInstanceManager, attacker: CharacterInstance, defender: CharacterInstance, attack_entry: Dict[str, Any], ctx: DamageContext) -> Dict[str, Any]:
    ...
    return {
        ...
        "crit_def_correction_54": crit_info["crit_def_correction_54"],
