...
class BattleOpStateManager:
    ...
    def _apply_buff_effect(
        ...
        if int(buff_id) == 54 and int(sub_id) == 1205001:
            applied = 0
            for team_type, signed_value in (("我方前排", float(value)), ("敌方", -float(value))):
                for pos in (0, 1, 2):
                    ...
                    inst.buffs = [
                        b
                        for b in (inst.buffs or [])
                        if not (int(getattr(b, "buff_id", 0)) == 54 and int(getattr(b, "sub_id", 0)) == 1205001)
                    ]
        ...
        for team_type, pos in targets:
            ...
            if special_id == 57 and int(sub_id) in (2, 4):
                char_manager.add_character_buff(
                    team_type=team_type,
                    position=pos,
                    buff_id=1,
