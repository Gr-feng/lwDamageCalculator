...
class BattleOpStateManager:
    ...
    def _apply_buff_effect(
        ...
        if target_type == 1:
            ...
            targets = [("敌方", int(enemy_target_pos_idx))]
        elif target_type == 4:
            ...
            if special_id == 6:
                ...
                def _apply_to_same_inst(eff):
                    ...
                    if _bid == 4:
                        ...
                        return
                    if _bid == 5:
                        inst.update_spirit(float(_val) / 20.0, mode="add")
                        return
                    if _bid == 6:
                ...
                # 根据 barrier_status 种类触发
                # 0/2/4：无效果
                if bs_kind == 6:
                    for _sid in (1, 3, 8, 10):
                        _apply_to_same_inst([1, _sid, 1, 1, 1])
                elif bs_kind == 5:
                    _apply_to_same_inst([5, 0, 1, 1, 4])
                elif bs_kind == 7:
                    for _sid in (2, 4, 9, 11):
                        _apply_to_same_inst([1, _sid, 1, 1, 1])
                elif bs_kind == 8:
                    for _sid in (5, 6, 7):
                        _apply_to_same_inst([1, _sid, 1, 1, 1])
                elif bs_kind == 3:
                    # 将“实际附加数量 changed”作为 value，再次对敌方目标施加同种异常
                    # 这里按系统目标口径：target_type=3（敌方单体）
                ...
                # kind=1 已在上面处理；其余 kind 默认不做额外处理
                continue
